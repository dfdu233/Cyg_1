Component({
  ...
  methods: {
      // 搜索下拉框关键词
    searchOptions(e) {
      const keywords = e.detail.value.toLowerCase(); // 获取输入框的值，并转为小写字母
      const options = this.properties.nameList; // 获取传进来的下拉框选项数组
      const filteredOptions = options.filter((option) => {
        // 使用关键词筛选选项，可以根据自己的需求进行修改筛选逻辑
        return option.name.toLowerCase().indexOf(keywords) > -1;
      });
      this.setData({
        selectcontent: filteredOptions,
      });
    },
  // 下拉框收起和展开
   startChange(e) {
